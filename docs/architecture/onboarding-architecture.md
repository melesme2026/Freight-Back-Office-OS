...
- sales / referral workflows later
...
Drivers must often be created before true go-live, especially if document ingestion depends on them.
...
If onboarding is mixed directly into load processing:
	•	readiness becomes unclear
	•	billing setup is forgotten
...
Even if V1 is for your uncle’s workflow, the architecture must support many customers later.
