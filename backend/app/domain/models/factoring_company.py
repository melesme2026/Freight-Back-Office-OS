...
class FactoringCompany(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    default_reserve_percent: Mapped[Decimal] = mapped_column(
        Numeric(6, 3), nullable=False, default=Decimal("0"), server_default="0"
    )
    default_fee_percent: Mapped[Decimal] = mapped_column(
        Numeric(6, 3), nullable=False, default=Decimal("0"), server_default="0"
    )

    organization: Mapped[Organization] = relationship(
        back_populates="factoring_companies", lazy="selectin"
    )
    payment_records: Mapped[list[LoadPaymentRecord]] = relationship(
        back_populates="factoring_company", lazy="selectin"
    )
