...
def create_service_plan(
    *,
    ...
    normalized_currency_code = _normalize_currency_code(currency_code) or "USD"
    ...
    item = ServicePlan(
        ...
        base_price=_parse_decimal(base_price, "base_price"),
        currency_code=normalized_currency_code,
        per_load_price=_parse_optional_decimal(per_load_price, "per_load_price"),
        per_driver_price=_parse_optional_decimal(per_driver_price, "per_driver_price"),
        is_active=is_active,
    )
...
def update_service_plan(
    ...
    *,
    ...
    normalized_code = _normalize_code(code) if code is not None else None
    ...
    if base_price is not None:
        item.base_price = _parse_decimal(base_price, "base_price")
    if currency_code is not None:
        item.currency_code = _normalize_currency_code(currency_code) or item.currency_code
    if per_load_price is not None:
        item.per_load_price = _parse_optional_decimal(
            per_load_price, "per_load_price"
        )
    if per_driver_price is not None:
        item.per_driver_price = _parse_optional_decimal(
            per_driver_price, "per_driver_price"
        )
