...
def _staff_payload(org_id: str) -> dict[str, str]:
    return {
        ...
        "role": "admin",
        "staff_user_id": "00000000-0000-0000-0000-000000001111",
    }
...
def test_launch_smoke_owner_admin_and_driver_flow(db_session):
    ...
    db_session.add(
        Driver(
            ...
            full_name="Driver Smoke",
            phone="5551234",
            email="driver-smoke@example.com",
            is_active=True,
        )
    )
    db_session.flush()
    ...
    for idx, doc_type in enumerate(
        ["rate_confirmation", "bill_of_lading", "proof_of_delivery"], start=1
    ):
    ...
    invoice_resp = download_load_invoice(
        load_id=load.id, token_payload=_staff_payload(org_id), db=db_session
    )
    assert invoice_resp.media_type == "application/pdf"

    invoice_docs, _ = docs.list_documents(
        organization_id=org_id, load_id=str(load.id), document_type="invoice", page=1, page_size=20
    )
    assert len(invoice_docs) >= 1

    packet_resp = create_submission_packet(
        load_id=load.id,
        payload=SubmissionPacketCreateRequest(notes="launch smoke"),
        token_payload=_staff_payload(org_id),
        db=db_session,
    )
    packet_id = packet_resp.data["id"]

    zip_resp = download_submission_packet_zip(
        load_id=load.id, packet_id=packet_id, token_payload=_staff_payload(org_id), db=db_session
    )
    ...
    sent_resp = mark_submission_packet_sent(
        ...
        payload=SubmissionPacketMarkSentRequest(
            ...
            destination_name="Demo Broker",
            destination_email="broker@example.com",
            notes="emailed",
        ),
        token_payload=_staff_payload(org_id),
        db=db_session,
    ...
    followups = generate_followups_for_load(
        load_id=str(load.id), db=db_session, token_payload=_staff_payload(org_id)
    )
    ...
    upload = UploadFile(
        filename="driver-pod.pdf",
        file=BytesIO(b"driver-pod"),
    ...
    own_items, own_total = LoadService(db_session).list_loads(
        organization_id=org_id, driver_id=driver_id, page=1, page_size=20
    )
    assert own_total == 1
    ...
    other_load = LoadService(db_session).create_load(
        organization_id=org_id,
        customer_account_id=customer_id,
        driver_id=other_driver_id,
        load_number="SMK-002",
        pickup_location="LA",
        delivery_location="SF",
    )
