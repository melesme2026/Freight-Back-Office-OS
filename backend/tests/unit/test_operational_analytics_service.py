    ...
    db_session.add_all(
        [
            Driver(
                ...
                full_name="Ava Driver",
                phone="555-0101",
            ),
            Driver(
                id=uuid.UUID(DRIVER_2_ID),
                organization_id=ORG_ID,
                customer_account_id=CUSTOMER_ID,
                full_name="Ben Driver",
                phone="555-0102",
            ),
            Broker(
...
def _payment(
    ...
    *,
    ...
    reconciliation_status: FactoringReconciliationStatus = (
        FactoringReconciliationStatus.UNRECONCILED
    ),
...
def test_operational_analytics_calculates_revenue_unpaid_aging_and_collections(db_session):
    ...
    partial_load = _make_load(
        db_session, load_number="PART", delivery_date=today - timedelta(days=20)
    )
    old_load = _make_load(
        db_session,
        load_number="OLD",
        delivery_date=today - timedelta(days=75),
        broker_id=BROKER_2_ID,
    ...
    _payment(
        db_session,
        load_id=paid_load.id,
        expected="1000",
        received="1000",
        status=LoadPaymentStatus.PAID,
        reconciliation_status=FactoringReconciliationStatus.RECONCILED,
    )
    _payment(
        db_session,
        load_id=partial_load.id,
        expected="2000",
        received="500",
        status=LoadPaymentStatus.PARTIALLY_PAID,
        factoring_used=True,
        factoring_status=FactoringWorkflowStatus.FUNDED,
        reserve_amount="300",
    )
    _payment(
        db_session,
    ...
    bucket_60 = next(
        bucket for bucket in data["aging_report"]["buckets"] if bucket["bucket"] == "60_plus"
    )
...
def test_operational_analytics_groups_brokers_drivers_lanes_and_filters(db_session):
    ...
    _payment(
        ...
        factoring_used=True,
        factoring_status=FactoringWorkflowStatus.SUBMITTED_TO_FACTORING,
    )

    filtered = OperationalAnalyticsService(db_session).get_operational_analytics(
        org_id=ORG_ID, broker_id=BROKER_2_ID
    )
...
def test_operational_analytics_excludes_other_org_and_honors_date_range(db_session):
    ...
    data = OperationalAnalyticsService(db_session).get_operational_analytics(
        org_id=ORG_ID, date_from=today - timedelta(days=30), date_to=today
    )
